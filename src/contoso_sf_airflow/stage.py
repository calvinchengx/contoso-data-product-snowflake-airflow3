...
# `parents[2]` is the repository root from `src/<pkg>/stage.py`. The platform
# always sets PRODUCT_STAGE, so this fallback exists for a lone clone and for
# tests -- but a fallback that silently pointed INSIDE src/ would be a stage
# nothing else can see, which is the failure this module exists to prevent.
ROOT = pathlib.Path(__file__).resolve().parents[2]
STAGE = pathlib.Path(os.environ.get("PRODUCT_STAGE") or ROOT / "stages")
