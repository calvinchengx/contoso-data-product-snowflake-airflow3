...
from contoso_sf_airflow.sources import ERP_CONN, stream
from contoso_sf_airflow.stage import STAGE
...
def watermark(consumer: Consumer, topic: str, *, ready_timeout: float = 180.0) -> int:
...
def settled(consumer: Consumer, topic: str, polls: int = 3, gap: float = 5.0) -> int:
...
def surviving_customers() -> int:
    ...
    # The vendor's own dev credential, declared in its sources.yaml and carried
    # into the connection by the platform. Not a Contoso secret this product
    # holds: reading a count is a consumer verifying what it consumed.
    #
    # SERVICE NAME, NOT A PUBLISHED PORT. The Tasks cell reaches this on
    # localhost:55436 because its steps run on the host; here the worker is on
    # the compose network and a published port would either not resolve or --
    # worse -- resolve to the SIBLING stack's ERP.
    c = connection(ERP_CONN)
    dsn = (
        f"host={c['db_host']} port={c.get('db_port', 5432)} dbname={c['db_name']} "
...
def main() -> int:
    # RESOLVED HERE, not at import. A module-level lookup runs at DAG PARSE
    # time -- in the dag-processor, which has no reason to hold a broker
    # address and may be parsing before the platform has provisioned one.
    bootstrap, topic = stream(ERP_CONN)
    consumer = Consumer({
