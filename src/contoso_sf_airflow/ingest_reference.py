...
import requests
from contoso_sf_airflow.credentials import resolve

from contoso_sf_airflow.sources import REFERENCE_CONN, REFERENCE_KEY_SECRET, api
...
def fetch(path: str, key: str) -> requests.Response:
    return requests.get(
        f"{api(REFERENCE_CONN)}{path}", headers={"X-Api-Key": key}, timeout=600
    )
